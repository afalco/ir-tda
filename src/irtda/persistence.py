...
def superlevel_persistence(
    ...
    """Degree-0 diagram of the superlevel-set filtration, in ``-f`` coordinates.

    Equivalent to ``sublevel_persistence(-f)``. Components are born at local
    *maxima* (spectral bands) and die at local minima. The returned diagram
    lives in ``-f`` units, so ``birth = -band_height`` and ``persistence`` is
    the prominence of the band, expressed in the original units.
    """
    return sublevel_persistence(
...
def lifetime_diagram(diagram: np.ndarray) -> np.ndarray:
    """Map ``(birth, death)`` to the lifetime coordinates ``(birth, death - birth)``.

    This is the bijection ``T(a, b) = (a, b - a)`` of Frahi et al. (2020, Eq. 5).
    """
    if diagram.size == 0:
        return diagram.reshape(-1, 2)
    return np.column_stack([diagram[:, 0], diagram[:, 1] - diagram[:, 0]])
